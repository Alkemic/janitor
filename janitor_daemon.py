...
from janitor.daemon import JanitorDaemon
import config

if __name__ == '__main__':
    daemon = JanitorDaemon(
        config.PID_FILE,
        stdout='%s/log/stdout.log' % config.PROJECT_ROOT,
        stderr='%s/log/stderr.log' % config.PROJECT_ROOT,
    )
    ...
    if len(sys.argv) == 2:
        if 'start' == sys.argv[1]:
            ...
            print 'Running daemon \'%s\' in foreground' % sys.argv[0]
            daemon.run()
        else:
